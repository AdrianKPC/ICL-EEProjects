...
void get_subsq(int subs, const std::vector<int>& in, std::vector<int>& out){
   	...
   	for (int j = subs - subs%subsq_side ;j < subs - subs%subsq_side + subsq_side; j++)		{ 	for (int i = subsq_side*(subs%subsq_side) ; i < 					subsq_side*(subs%subsq_side) + subsq_side   ; i++){
            		out.push_back( in[ twod_to_oned(j, i, side) ]  );
       		}
   	}
